...
# ---- dataset paths ------
data_dir = "datasets/SimpleQuestions_v2/"
...
emb_vocab, emb_vecs = read_embedding(embed_pt_filepath)
...
for i in range(vocab_size):
    ...
    if emb_vocab.contains(word):
        vecs[i] = emb_vecs[emb_vocab.index(word)]
    else:
        num_unk += 1
        vecs[i].uniform_(-0.05, 0.05)

print('unk count = {}'.format(num_unk))
emb_vocab = None
emb_vecs = None
...
print('train_file: {}, num train = {}\n'.format(train_file, train_dataset.size))
print('val_file: {}, num dev   = {}\n'.format(val_file, val_dataset.size))
print('test_file: {}, num test  = {}\n'.format(test_file, test_dataset.size))
