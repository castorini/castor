...
class SICKEvaluator(Evaluator):
    ...
    def get_scores(self):
        with torch.cuda.device(self.device):
            ...
            predict_classes = torch.arange(1, num_classes + 1).expand(self.batch_size, num_classes).cuda()
            test_kl_div_loss = 0
            predictions = []
            true_labels = []
            ...
            for batch in self.data_loader:
                ...
                if len(predict_classes) != len(batch.a):
                    predict_classes = torch.arange(1, num_classes + 1).expand(len(batch.a), num_classes).cuda()
...
class MSRVIDEvaluator(Evaluator):
    ...
    def get_scores(self):
        with torch.cuda.device(self.device):
            self.model.eval()
            num_classes = self.dataset_cls.NUM_CLASSES
            predict_classes = torch.arange(0, num_classes).expand(self.batch_size, num_classes).cuda()
            test_kl_div_loss = 0
            predictions = []
            true_labels = []
            ...
            for batch in self.data_loader:
                ...
                # handle last batch which might have smaller size
                if len(predict_classes) != len(batch.a):
                    predict_classes = torch.arange(0, num_classes).expand(len(batch.a), num_classes).cuda()
                true_labels.append((predict_classes * batch.label.data).sum(dim=1))
                predictions.append((predict_classes * output.data.exp()).sum(dim=1))
